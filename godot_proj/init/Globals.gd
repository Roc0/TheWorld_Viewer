...
func _ready():
	var w = get_tree().get_root().find_node("TheWorld_MainNode", true, false)
	GDN_main().init(w)
	...
	#_result = GDN_viewer().set_initial_world_viewer_pos(1195425.176295, 5465512.560295, 0)
...
func exitFunct():
	debug_print(self.name + ": exitFunct!")
	#GDN_main.destroy()
	#GDN_main().free()
